        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="bi bi-pencil me-2"></i>Edit FAQ</h5>
                    ...
                    <form method="POST" action="{{ route('admin.faq.update', $faq->id) }}">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="category" id="category" class="form-select @error('category') is-invalid @enderror" required>
                                    ...
                                    <option value="products" {{ old('category', $faq->category) == 'products' ? 'selected' : '' }}>Products</option>
                                    <option value="orders" {{ old('category', $faq->category) == 'orders' ? 'selected' : '' }}>Orders</option>
                                    <option value="payment" {{ old('category', $faq->category) == 'payment' ? 'selected' : '' }}>Payment</option>
                                    <option value="shipping" {{ old('category', $faq->category) == 'shipping' ? 'selected' : '' }}>Shipping</option>
                                    <option value="returns" {{ old('category', $faq->category) == 'returns' ? 'selected' : '' }}>Returns</option>
                                </select>
                                @error('category')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-floppy"></i> Update FAQ
                            </button>
                        </div>
                    </form>
                </div>
